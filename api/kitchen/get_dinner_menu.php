...
if($_POST['token'] == API_TOKEN)
{
	if(isset($kitchen_id) && $kitchen_id > 0)
	{
		...
		foreach($category_array as $category){
			...
			$res = $db->pdoQuery("SELECT * FROM mastermenu WHERE userid = '".$kitchen_id."' AND cuisinetype=0 AND menutype=2 AND category='".$category."'")->results();
			if(count($res) > 0)
			{
		...
		foreach($category_array as $category){
			...
			$res = $db->pdoQuery("SELECT * FROM mastermenu WHERE userid = '".$kitchen_id."' AND cuisinetype=1 AND menutype=2 AND category='".$category."'")->results();
			if(count($res) > 0)
			{
				foreach ($res as $key => $value) {
		...
		foreach($category_array as $category){
			...
			$res = $db->pdoQuery("SELECT * FROM mastermenu WHERE userid = '".$kitchen_id."' AND cuisinetype=2 AND menutype=2 AND category='".$category."'")->results();
			if(count($res) > 0)
			{
				foreach ($res as $key => $value) {
