...
if ($_POST['token'] == API_TOKEN) {
	if ($kitchen_id > 0) {
		...
		$res = $db->pdoQuery("SELECT o.id,oi.id as orderitemsid,c.kitchenname,c.mobilenumber,o.deliveryaddress,o.orderid,o.netamount,o.status,oi.delivery_date,
                                o.orderdate,o.ordertype,oi.status as itemstatus
							  FROM orders AS o
							  ...
							  ORDER BY oi.order_id DESC,oi.delivery_date DESC
                            ")->results();
		...
		if (count($res) > 0) {
			foreach ($res as $key => $value) {
				...
				if ($value['ordertype'] == 'package') {
					...
					if ($value['itemstatus'] == 0) {
						$status = "Ready to pick";
					} else if ($value['itemstatus'] == 1) {
						$status = "Assign to Rider";
					} else if ($value['itemstatus'] == 2) {
						$status = "Start delivery";
					} else if ($value['itemstatus'] == 5 && $value['status'] == 1) {
						$status = "Order in Preparation";
					} else if ($value['itemstatus'] == 5 && $value['status'] == 0) {
						$status = "Pending";
					}
				} else {
