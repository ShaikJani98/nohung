function resetdata(){
    ...
    $("#countryname_div").removeClass("has-error is-focused");
  ...
  function checkvalidation(btntype){
    ...
    var isvalidcountryname = isvalidsortname = isvalidphonecode = 0 ;
    
    PNotify.removeAll();
    if(countryname == ''){
      $("#countryname_div").addClass("has-error is-focused");
      new PNotify({title: 'Please enter region / area name !',styling: 'fontawesome',delay: '3000',type: 'error'});
      isvalidcountryname = 0;
    }else {
      if(countryname.length<3){
        $("#countryname_div").addClass("has-error is-focused");
        new PNotify({title: 'Region / area name require minimum 3 characters !',styling: 'fontawesome',delay: '3000',type: 'error'});
        isvalidcountryname = 0;
      }else{
        isvalidcountryname = 1;
      }
    }
    if(sortname == ''){
    ...
    if(isvalidcountryname == 1 && isvalidsortname == 1 && isvalidphonecode == 1){
  
      var formData = new FormData($('#countryform')[0]);
      if(ACTION==0){
        var uurl = SITE_URL+"country/country-add";
